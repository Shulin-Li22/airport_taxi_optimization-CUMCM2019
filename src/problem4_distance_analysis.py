...
def calculate_trip_distances():
    ...
    # 读取 Excel 文件
    file_path = 'data/processed/Taxi_Trips999.xlsx'  # 原来是 '~/Downloads/Taxi_Trips999.xlsx'
...
def visualize_distance_analysis(df, threshold, distance_percentiles):
    ...
    plt.hist(df['距离(km)'], bins=50, color='gray', edgecolor='black', alpha=0.7)
    plt.axvline(x=threshold, color='red', linestyle='--', linewidth=2,
                label=f'{threshold:.2f} km threshold')
    ...
    plt.plot(sorted_distances, cdf, color='brown', linewidth=2)
    plt.axvline(x=threshold, color='red', linestyle='--', linewidth=2,
                label=f'{threshold:.2f} km threshold')

    # 添加分位数点
    for i, (perc, value) in enumerate(zip([25, 50, 75], distance_percentiles[:3])):
        plt.plot(value, perc / 100, 'o', markersize=8,
                 label=f'{perc}th percentile: {value:.2f} km')

    plt.title('Cumulative Distribution of Trip Distances')
    plt.xlabel('Trip Distance (km)')
...
def save_results(df):
    ...
    # 保存结果到新的 Excel 文件
    output_file = 'data/processed/Taxi_Trips标记.xlsx'  # 原来是 '~/Downloads/Taxi_Trips标记.xlsx'
